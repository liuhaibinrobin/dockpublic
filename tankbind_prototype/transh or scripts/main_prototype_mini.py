...
class PairwiseLoss(nn.Module):
    def __init__(self, keep_rate=1., sigmoid_lambda=0.5,
                 ingrp_thr=1, outgrp_thr=9999, eval=False):
...
def main(args):
    ...
    train_sampler = SessionBatchSampler(train_dataset, seed=0, name=timestamp, index_save_path="/home/jovyan/main_tankbind/training_information_prototype/small_train")
    ...
    for epoch in range(args.num_epochs):
        ...
        if True: # Alignment
            ...
            for data in tqdm(train_dataloader):
                ...
                sessions = data.session_au
                
                if args.use_affinity_mask:
                    affinity_pred = affinity_pred[data.real_affinity_mask]
                    affinity_true = affinity_true[data.real_affinity_mask]


                loss, recto_rate = pairwiseloss(affinity_pred, affinity_true)
                print(f"|===============|================")
                print(f"| affinity_pred | {affinity_pred}")
                print(f"| affinity_true | {affinity_true}")
                ...
                # loss_result.append([loss.detach().cpu(), data.cpu().ids, data.cpu().session])
...
if __name__ == "__main__":
    ...
    # Model configuration
    parser.add_argument("--pair_threshold", type=int, default=-1,
