...
from data_prototype import get_full_data_prototype, get_internal_data
...
def main(args):
    ...
    internal_dataset = get_internal_data(input_path)
    print("fin!")
    logging.info(
    ...
    print("Creating valtest dataloaders...", end="")
    ...
    internal_dataloader = DataLoader(internal_dataloader, batch_size=valid_batch_size, follow_batch=['x', 'compound_pair'], shuffle=False, pin_memory=False, num_workers=num_workers)
    print("fin!")
    ...
    for epoch in range(args.num_epochs):
        ...
        if True: # Alignment
            ...
            torch.save({"affinity_true": affinity_true, "affinity_pred": affinity_pred, "loss": loss, "session_list": session_list, "sample_id_list": sample_id_list}, f"{pre}/train/epoch_result/epoch_{epoch}.pt")
        
        run_train(pre=pre)

        run_validation(pre=pre, label="iid", dataset=iid_dataset, dataloader=iid_dataloader, epoch=epoch, 
                       model=model, pairwiseloss=pairwiseloss, device=device, writer=writer,  logging=logging)
        ...
        run_validation(pre=pre, lable="test", dataset=test_dataset, dataloader=test_dataloader, epoch=epoch, 
                       model=model, pairwiseloss=pairwiseloss, device=device, writer=writer,  logging=logging)
        ...
        with torch.no_grad():
            ...
            for session in validation_info.session_au.unique():
                ...
                validation_len += np.sqrt(num_pairs)
                validation_loss += loss * np.sqrt(num_pairs)
                validation_recto_rate += recto_rate * np.sqrt(num_pairs)
                
                validation_result.append([session, session_len, num_pairs, loss.item(), recto_rate.item()])
                # end of sample scope: ratio and loss calculate
            
            
        # save result as .csv
        validation_result = pd.DataFrame(validation_result, columns=["session", "length", "num_pairs", "loss", "recto_rate"])
...
def run_train(pre, dataset, dataloader, 
