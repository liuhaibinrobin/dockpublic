        ...
        for _epoch in range(args.max_epoch):
            ...
            for i,data in tqdm(enumerate(train_data_loader), total=len(train_data_loader)):
                ...
                if cfg_mode == "tankbind": # Results : affinity_pred_dictbb
                    ...
                    loss.backward()
                    optimizer.step()
                    epoch_loss_list.append(loss.detach().cpu())
                    #epoch_nci_list.append(nci_pred.detach().cpu())
            ...
            nci_dict[f"train_{_epoch}"] = epoch_nci_list
             
            
            losst = torch.mean(torch.cat([torch.tensor([i]) for i in epoch_loss_list]), dim=0)
            ...
            if val_data_loader is not None:
                with torch.no_grad():
                    ...
                    losst_va = torch.mean(torch.cat([torch.tensor([i]) for i in epoch_loss_list]), dim=0)
                    #score_va = torch.mean(torch.cat([torch.tensor([i]) for i in _list_score_va]), dim=0)
                    
                    nci_dict[f"val_{_epoch}"] = epoch_nci_list
            ...
            with torch.no_grad():
               if test_data_loader is not None:
                    ...
                    #nci_dict[f"test_{_epoch}"] = epoch_nci_list
                    losst_te = torch.mean(torch.cat([torch.tensor([i]) for i in epoch_loss_list]), dim=0)
...
if __name__=="__main__":
    ...
    parser.add_argument("--data_version", type=str, default='v8.30')    
    ...
    parser.add_argument("--max_node", type=int, default=2000)
