...
def main(rank, size, args):
    ...
    save_files_path = f"/home/jovyan/BetaBind/ours/Inputs/Savedfiles/{cfg_mode}.{cfg_data_version}/"
        ...
        for _epoch in range(args.max_epoch):
            ...
            for i,data in tqdm(enumerate(train_data_loader), total=len(train_data_loader)):
                ...
                if cfg_mode == "tankbind": # Results : affinity_pred_dictbb
                    ...
                    loss = model.module.calculate_loss(affinity_pred, y_pred, nci_pred, data.affinity, data.dis_map,
            ...
            txtlog.qrint('TRAIN----> Epoch [{}/{}], Loss: {:.4f}' .format(_epoch+1, args.max_epoch, losst.item()))
            #txtlog.qrint('TRAIN----> Epoch [{}/{}], Score: {:.4f}' .format(_epoch+1, args.max_epoch, scoret.item()))
