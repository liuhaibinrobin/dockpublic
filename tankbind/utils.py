...
def construct_data_from_graph_gvp(protein_node_xyz, protein_seq, protein_node_s,
                                  ...
                                  input_atom_edge_attr_list, includeDisMap=True, contactCutoff=8.0, pocket_radius=20,
                                  interactionThresholdDistance=10, compoundMode=1,
                                  add_noise_to_com=None, use_whole_protein=False, use_compound_com_as_pocket=True,
    ...
    input_edge_idx, input_protein_edge_s, input_protein_edge_v = get_protein_edge_features_and_index(protein_edge_index,
                                                                                                     protein_edge_s,
                                                                                                     protein_edge_v,
                                                                                                     keepNode)
    ...
    if compoundMode == 0:
        data['compound'].x = torch.tensor(compound_node_features,
                                          dtype=torch.bool)  # [num_compound_nodes, num_compound_feature]
        data['compound', 'c2c', 'compound'].edge_index = torch.tensor(input_atom_edge_list,
                                                                      dtype=torch.long).t().contiguous()
        c2c = torch.tensor(input_atom_edge_attr_list, dtype=torch.long)
        data['compound', 'c2c', 'compound'].edge_attr = F.one_hot(c2c - 1,
                                                                  num_classes=1)  # [num_edges, num_edge_features]
    elif compoundMode == 1:
...
def select_pocket_by_predicted_affinity(info):
    ...
    selected = info.query("not is_ligand_pocket or (base_pdb in @use_whole_protein_list)").sort_values(['base_pdb',
                                                                                                        'affinity_pred']).groupby(
        "base_pdb").tail(1).sort_values("index").reset_index(drop=True)
    return selected
...
def extract_list_from_prediction(info, y, y_pred, selected=None, smiles_to_mol_dict=None,
                                 coords_generated_from_smiles=False):
    idx = 0
    y_list = []
    y_pred_list = []
...
def evaulate_with_affinity(data_loader, model, criterion, affinity_criterion, relative_k, device, pred_dis=False,
                           info=None, saveFileName=None, use_y_mask=False, skip_y_metrics_evaluation=False):
    y_list = []
    y_pred_list = []
    affinity_list = []
    affinity_pred_list = []
...
def evaulate_affinity_only(data_loader, model, criterion, affinity_criterion, relative_k, device, info=None,
                           saveFileName=None, use_y_mask=False):
    y_list = []
    y_pred_list = []
    affinity_list = []
    affinity_pred_list = []
