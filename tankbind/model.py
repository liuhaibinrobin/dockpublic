...
class NFTNCIClassifier(torch.nn.Module):
    """
...
class NFTNet(torch.nn.Module):
    """
    Tankbind-based model enhanced with NCI classification.
    """
    ...
    def forward(self, data):
        ...
        ## z.shape: batch_size * padded_protein_len * padded_compound_len * hidden_channels
        z_mask = torch.einsum("bi,bj->bij", protein_out_mask, compound_out_mask)
        ## z_mask.shape: batch_size * padded_protein_len * padded_compound_len
        ...
        if self.readout_mode == 1:
            ...
            valid_interaction_z = (z * z_mask.unsqueeze(-1)).sum(axis=(1, 2)) / z_mask.sum(axis=(1, 2)).unsqueeze(-1)
            affinity_pred = self.linear_energy(valid_interaction_z).squeeze(-1)
            # print("z shape", z.shape, "z_mask shape", z_mask.shape,
            # "valid_interaction_z shape", valid_interaction_z.shape, "affinity_pred shape", affinity_pred.shape)
        if self.readout_mode == 2:
            pair_energy = (self.gate_linear(z).sigmoid() * self.linear_energy(z)).squeeze(-1) * z_mask
            affinity_pred = self.leaky(self.bias + ((pair_energy).sum(axis=(-1, -2))))
        else:
            raise ValueError(f"self.readout_mode {self.readout_mode} not found!")
...
def get_nft_model(mode, logging, device, **kwargs):
    if mode == 0:
        logging.info("5 stack, readout2, pred dis map add self attention and GVP embed, compound model GIN")
        model = NFTNet(kwargs).to(device)
    else:
        raise ValueError("Selected mode not defined!")
    return model
